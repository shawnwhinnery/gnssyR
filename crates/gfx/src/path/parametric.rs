    ...
    for seg in &path.segments {
        match seg {
            ...
            Segment::Arc {
                ...
                let arc_to =
                    *center + Vec2::new(radius * end_angle.cos(), radius * end_angle.sin());
...
fn eval_point(g: &SegGeom, t: f32) -> Vec2 {
    match &g.kind {
        ...
        SegKind::Cubic { cp1, cp2 } => {
            let s = 1.0 - t;
            s * s * s * g.from + 3.0 * s * s * t * *cp1 + 3.0 * s * t * t * *cp2 + t * t * t * g.to
        }
        SegKind::Arc {
            center,
            radius,
...
fn eval_tangent(g: &SegGeom, t: f32) -> Vec2 {
    match &g.kind {
        ...
        SegKind::Cubic { cp1, cp2 } => {
            let s = 1.0 - t;
            let d = 3.0
                * (s * s * (*cp1 - g.from) + 2.0 * s * t * (*cp2 - *cp1) + t * t * (g.to - *cp2));
            d.normalize_or_zero()
        }
        SegKind::Arc {
            center: _,
...
fn find_seg(geoms: &[SegGeom], target: f32) -> (&SegGeom, f32) {
    ...
    for (i, g) in geoms.iter().enumerate() {
        ...
        if next >= target || i == geoms.len() - 1 {
            let lt = if g.length < 1e-10 {
                0.0
            } else {
                ((target - acc) / g.length).clamp(0.0, 1.0)
            };
            return (g, lt);
        }
        acc = next;
...
fn split_quad(from: Vec2, cp: Vec2, to: Vec2, t: f32) -> ((Vec2, Vec2, Vec2), (Vec2, Vec2, Vec2)) {
